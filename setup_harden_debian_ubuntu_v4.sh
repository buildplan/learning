...
# like Docker, Tailscale with Headscale support, and system monitoring (SMTP and ntfy).
# It supports a configuration file for automation and is idempotent for safe re-runs.
#
# Prerequisites:
# - Run as root (e.g., sudo ./setup_harden_debian_ubuntu.sh).
...
#   Make executable: chmod +x setup_harden_debian_ubuntu.sh
#   Run interactively: sudo ./setup_harden_debian_ubuntu.sh
#   Run with config: sudo ./setup_harden_debian_ubuntu.sh --config /etc/setup_harden.conf
#   Run quietly: sudo ./setup_harden_debian_ubuntu.sh --quiet
#
# Config File:
#   Optional: /etc/setup_harden.conf or specify via --config <file>
#   Format (example):
#     USERNAME=ali
#     HOSTNAME=doVPS
#     SSH_PORT=5595
...
#
# Notes:
# - Logs to /var/log/setup_harden_debian_ubuntu_*.log.
# - Backups stored in /root/setup_harden_backup_*.
# - A reboot is recommended to apply changes.
# - Test in a VM before production use.
#
# Troubleshooting:
# - Check log file for errors.
# - Restore SSH config from /root/setup_harden_backup_*/sshd_config.backup_* if locked out.
# - Ensure disk space (>2GB) for swap.
...
SSH_SERVICE=""
ID="" # Populated from /etc/os-release
...
load_config() {
    ...
    if [[ -f "$config_file" ]]; then
        ...
        [[ -n "${USERNAME:-}" && -n "${HOSTNAME:-}" && -n "${SSH_PORT:-}" ]] || { print_error "Missing required fields (USERNAME, HOSTNAME, SSH_PORT) in config."; exit 1; }
        validate_username "$USERNAME" || { print_error "Invalid USERNAME in config."; exit 1; }
        validate_hostname "$HOSTNAME" || { print_error "Invalid HOSTNAME in config."; exit 1; }
        validate_port "$SSH_PORT" || { print_error "Invalid SSH_PORT in config."; exit 1; }
        [[ -n "${TIMEZONE:-}" ]] && validate_timezone "$TIMEZONE" || { print_error "Invalid TIMEZONE in config."; exit 1; }
        [[ -n "${SWAP_SIZE:-}" ]] && validate_swap_size "$SWAP_SIZE" || { print_error "Invalid SWAP_SIZE in config."; exit 1; }
        if [[ -n "${UFW_PORTS:-}" ]]; then
            for port in ${UFW_PORTS//,/ }; do
                validate_ufw_port "$port" || { print_error "Invalid UFW_PORTS format: $port."; exit 1; }
            done
        fi
        [[ -n "${AUTO_UPDATES:-}" && "$AUTO_UPDATES" =~ ^(yes|no)$ ]] || AUTO_UPDATES="no"
        [[ -n "${INSTALL_DOCKER:-}" && "$INSTALL_DOCKER" =~ ^(yes|no)$ ]] || INSTALL_DOCKER="no"
        [[ -n "${INSTALL_TAILSCALE:-}" && "$INSTALL_TAILSCALE" =~ ^(yes|no)$ ]] || INSTALL_TAILSCALE="no"
        if [[ -n "${TAILSCALE_LOGIN_SERVER:-}" ]]; then
            validate_url "$TAILSCALE_LOGIN_SERVER" || { print_error "Invalid TAILSCALE_LOGIN_SERVER in config."; exit 1; }
            [[ -n "${TAILSCALE_AUTH_KEY:-}" ]] || { print_error "TAILSCALE_AUTH_KEY required when TAILSCALE_LOGIN_SERVER is set."; exit 1; }
            [[ -n "${TAILSCALE_OPERATOR:-}" ]] && validate_username "$TAILSCALE_OPERATOR" || { print_error "Invalid TAILSCALE_OPERATOR in config."; exit 1; }
            [[ -n "${TAILSCALE_ACCEPT_DNS:-}" && "$TAILSCALE_ACCEPT_DNS" =~ ^(yes|no)$ ]] || TAILSCALE_ACCEPT_DNS="yes"
            [[ -n "${TAILSCALE_ACCEPT_ROUTES:-}" && "$TAILSCALE_ACCEPT_ROUTES" =~ ^(yes|no)$ ]] || TAILSCALE_ACCEPT_ROUTES="yes"
        fi
        if [[ -n "${SMTP_SERVER:-}" ]]; then
            [[ "$SMTP_SERVER" =~ ^[a-zA-Z0-9.-]+$ ]] || { print_error "Invalid SMTP_SERVER in config."; exit 1; }
            [[ -n "${SMTP_PORT:-}" ]] && validate_smtp_port "$SMTP_PORT" || { print_error "Invalid SMTP_PORT in config."; exit 1; }
            [[ -n "${SMTP_FROM:-}" ]] && validate_email "$SMTP_FROM" || { print_error "Invalid SMTP_FROM in config."; exit 1; }
            [[ -n "${SMTP_TO:-}" ]] && validate_email "$SMTP_TO" || { print_error "Invalid SMTP_TO in config."; exit 1; }
        fi
        if [[ -n "${NTFY_SERVER:-}" ]]; then
            validate_url "$NTFY_SERVER" || { print_error "Invalid NTFY_SERVER in config."; exit 1; }
            [[ -n "${NTFY_TOKEN:-}" ]] && validate_ntfy_token "$NTFY_TOKEN" || { print_error "Invalid NTFY_TOKEN in config."; exit 1; }
        fi
        return 0
    else
...
collect_config() {
    ...
    if [[ -n "$CONFIG_FILE" || -f "/etc/setup_harden.conf" ]]; then
        ...
        if [[ "$INSTALL_TAILSCALE" == "yes" ]]; then
            ...
            if [[ -n "$TAILSCALE_LOGIN_SERVER" ]]; then
                validate_url "$TAILSCALE_LOGIN_SERVER" || { print_error "Invalid Tailscale login server URL."; exit 1; }
                ...
                TAILSCALE_OPERATOR=${TAILSCALE_OPERATOR:-$USERNAME}
                validate_username "$TAILSCALE_OPERATOR" || { print_error "Invalid Tailscale operator username."; exit 1; }
                TAILSCALE_ACCEPT_DNS="yes"
        ...
        if confirm "Configure system monitoring with SMTP and/or ntfy?"; then
            ...
            [[ "$SMTP_SERVER" =~ ^[a-zA-Z0-9.-]+$ ]] || { print_error "Invalid SMTP server."; exit 1; }
            read -rp "$(echo -e "${CYAN}Enter SMTP port [587]: ${NC}")" SMTP_PORT
            SMTP_PORT=${SMTP_PORT:-587}
            validate_smtp_port "$SMTP_PORT" || { print_error "Invalid SMTP port (use 25,2525,8025,587,80,465,8465,443)."; exit 1; }
            read -rp "$(echo -e "${CYAN}Enter SMTP from email address: ${NC}")" SMTP_FROM
            validate_email "$SMTP_FROM" || { print_error "Invalid SMTP from email."; exit 1; }
            read -rp "$(echo -e "${CYAN}Enter SMTP to email address: ${NC}")" SMTP_TO
            validate_email "$SMTP_TO" || { print_error "Invalid SMTP to email."; exit 1; }
            read -rp "$(echo -e "${CYAN}Enter ntfy server URL (e.g., https://ntfy.mydomain.com/ovps, press Enter to skip): ${NC}")" NTFY_SERVER
            if [[ -n "$NTFY_SERVER" ]]; then
                validate_url "$NTFY_SERVER" || { print_error "Invalid ntfy server URL."; exit 1; }
                read -rp "$(echo -e "${CYAN}Enter ntfy token: ${NC}")" NTFY_TOKEN
                validate_ntfy_token "$NTFY_TOKEN" || { print_error "Invalid ntfy token (must start with tk_)."; exit 1; }
            fi
        fi
    fi
...
EOF
            if [[ -f /etc/postfix/main.cf ]] && grep -q "relayhost = \[$SMTP_SERVER\]:$SMTP_PORT" /etc/postfix/main.cf; then
                ...
                read -sp "$(echo -e "${CYAN}Enter SMTP2go username: ${NC}")" SMTP_USER
                echo
                read -sp "$(echo -e "${CYAN}Enter SMTP2go password: ${NC}")" SMTP_PASS
                echo
        ...
        print_info "Configuring disk space monitoring cron job..."
        NEW_CRON_CONFIG=$(mktemp)
        tee "$NEW_CRON_CONFIG" > /dev/null <<EOF
0 0 * * * root df -h / | grep -q '[8-9][0-9]%\\|100%' && { \
    [[ -n "${SMTP_TO:-}" ]] && echo "Disk usage critical on $SERVER_NAME: \$(df -h / | tail -n 1)" | mail -s "Disk Alert" "$SMTP_TO"; \
    [[ -n "${NTFY_SERVER:-}" ]] && curl -u ":$NTFY_TOKEN" -H "Title: Disk Space Alert on $SERVER_NAME" -H "Tags: warning" -d "Disk usage critical: \$(df -h / | tail -n 1)" "$NTFY_SERVER" > /dev/null 2>&1; \
}
EOF
        if [[ -f /etc/cron.d/setup_harden_monitoring ]] && cmp -s "$NEW_CRON_CONFIG" /etc/cron.d/setup_harden_monitoring; then
            print_info "Disk space cron job already configured. Skipping."
        else
            mv "$NEW_CRON_CONFIG" /etc/cron.d/setup_harden_monitoring
            chmod 644 /etc/cron.d/setup_harden_monitoring
            print_success "Disk space monitoring cron job configured."
        fi

        print_info "Configuring rsync backup cron job..."
        NEW_RSYNC_CRON=$(mktemp)
        tee "$NEW_RSYNC_CRON" > /dev/null <<EOF
20 6 * * * root /usr/bin/rsync -avz --partial --delete --exclude '.ssh/' -e 'ssh -p 23' /home/$USERNAME/ u457300-sub4@u457300.your-storagebox.de:/home/ovps/ > /var/log/backup_rsync.log 2>&1 && { \
    [[ -n "${SMTP_TO:-}" ]] && echo "Rsync backup completed on $SERVER_NAME" | mail -s "Backup Complete" "$SMTP_TO"; \
    [[ -n "${NTFY_SERVER:-}" ]] && curl -u ":$NTFY_TOKEN" -H "Title: Backup rsync doVPS to Hetzner" -H "Tags: white_check_mark" -d "rsync executed on $SERVER_NAME" "$NTFY_SERVER" > /dev/null 2>&1; \
}
EOF
        if [[ -f /etc/cron.d/setup_harden_rsync ]] && cmp -s "$NEW_RSYNC_CRON" /etc/cron.d/setup_harden_rsync; then
            print_info "Rsync backup cron job already configured. Skipping."
        else
            mv "$NEW_RSYNC_CRON" /etc/cron.d/setup_harden_rsync
            chmod 644 /etc/cron.d/setup_harden_rsync
            print_success "Rsync backup cron job configured."
        fi
    ...
    if [[ -n "$existing_swap" ]]; then
        ...
        if [[ -n "${SWAP_SIZE:-}" ]]; then
            if [[ "$current_size" != "$SWAP_SIZE" ]]; then
                ...
                print_info "Swap file already at $SWAP_SIZE."
            fi
        elif confirm "Modify existing swap file size?"; then
            while true; do
        ...
        elif confirm "Configure a swap file (recommended for < 4GB RAM)?"; then
            while true; do
...
generate_summary() {
    ...
    for service in "$SSH_SERVICE" fail2ban chrony ufw; do
        if systemctl is-active --quiet "$service"; then
            print_success "Service $service is active."
        else
            print_error "Service $service is not active. Please check 'systemctl status $service'."
        fi
    done
    if [[ "$INSTALL_DOCKER" == "yes" ]]; then
        if systemctl is-active --quiet docker; then
            ...
            print_error "Service docker is not active. Please check 'systemctl status docker'."
        fi
    fi
    if [[ "$INSTALL_TAILSCALE" == "yes" ]]; then
        if systemctl is-active --quiet tailscaled; then
            ...
            print_error "Service tailscaled is not active. Please check 'systemctl status tailscaled'."
        fi
    fi
    if [[ -n "${SMTP_SERVER:-}" ]]; then
        if systemctl is-active --quiet postfix; then
            print_success "Service postfix is active."
        else
            print_error "Service postfix is not active. Please check 'systemctl status postfix'."
        fi
    fi

    echo -e "\n${GREEN}Server setup and hardening script has finished successfully.${NC}" | tee -a "$LOG_FILE"
    echo -e "\n${YELLOW}Configuration Summary:${NC}"
    ...
    if [[ -n "${SMTP_SERVER:-}" || -n "${NTFY_SERVER:-}" ]]; then
        echo -e "  - Monitoring:       Check /var/log/mail.log or ntfy notifications"
    fi
